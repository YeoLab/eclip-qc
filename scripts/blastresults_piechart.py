...
for index,row in save_n_dict.iterrows():
    if row['sseq_name'] == "No_Species":
        row['sseq_name'] = row['sseqid']
...
for index,row in save_x_dict.iterrows():
    if row['sseq_name'] == "No_Species":
        row['sseq_name'] = row['sseqid']
...
# origin_blastx2 = pd.read_csv(output_x_csv)
...
ax7.set_title('Top Six Hits For Blast_n')
...
ax8.set_title('Top Six Hits For Blast_x')
